...
2. **Sonic Burst:** The sensor automatically sends an 8-cycle burst of 40kHz sound.
3. **Echo:** The sensor sends an electrical pulse back to the Arduino. The **duration** of this electrical pulse is the sound's total round-trip travel time!

::: {.callout-tip}
**Think of it like a Speaker and a Microphone!**
- **Trig (Trigger) = Speaker:** The Arduino OUTPUTS a command here to tell the sensor to shout.
- **Echo = Microphone:** The Arduino INPUTS the data from here, listening for the sensor to reply.
:::
...
## The Helper Function 🛠️

Instead of repeating the pinging code multiple times, we put it in a function!
...
int measureDistance() {
  ...
  return distance; // Send the result back!
}
```
